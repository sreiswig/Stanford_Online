...
def get_icl_prompts(
    ...
    **Note**: Be sure to shuffle the support examples and labels 
    ...
    ### START CODE HERE ###
    shuffled_indices = np.random.permutation(len(support_inputs))
    for index in shuffled_indices:
        if prompt_mode == "qa":
            prompt += support_inputs[index] +  " In the " + support_labels[index] + ". "
        elif prompt_mode == "tldr":
            prompt += support_inputs[index] + " TL;DR: " + support_labels[index]
        elif prompt_mode == "custom":
            prompt += support_inputs[index] + " !> " + support_labels[index] # made up summarize symbol
        else:
            prompt += support_inputs[index] + " " + support_labels[index]
    ...
    if prompt_mode == "qa":
        ...
        prompt += " " + test_input + " !>" # made up summarize symbol
    else:
        prompt += " " + test_input  
    ### END CODE HERE ###
                ...
                for k in ks:
                    ...
                    for repeat in range(repeats):
                        ...
                        for row in pbar:
                            ...
                            ### START CODE HERE ###
                            prompt = get_icl_prompts(support_inputs=support_x, support_labels=support_y, test_input=test_input, prompt_mode=prompt_mode)
                            tokens = tokenizer.encode(prompt, return_tensors='pt').to(DEVICE)
                            samples = do_sample(model=model, input_ids=tokens, stop_tokens=stop_tokens, max_tokens=max_tokens)
