    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            with torch.no_grad():
                ...
                out = self._network(images_support)
                
                # Calculate prototypes from features
                nunique_labels = torch.unique(labels_support).shape[0]
                K = images_support.shape[0]//nunique_labels
                centroids = out.reshape(nunique_labels, K, -1).mean(dim=1)

                # Calculate support logits
                support_logits = - 1 * ((out.unsqueeze(1) - centroids.unsqueeze(0))**2).sum(dim = -1)
            
            # Generate query features
            query_feats = self._network(images_query)
                
            # Calculate query logits
            query_logits = - 1 * ((query_feats.unsqueeze(1) - centroids.unsqueeze(0))**2).sum(dim = -1)

            # Compute loss
            loss_batch.append(torch.nn.functional.cross_entropy(query_logits, labels_query))

            # Compute accuracies for support and query sets
