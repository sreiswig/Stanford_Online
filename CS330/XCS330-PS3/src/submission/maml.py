...
class MAML:
    ...
    def _inner_loop(self, images, labels, train):
        ...
        ### START CODE HERE ###
        # adapt for _num_inner_steps steps
        for _ in range(self._num_inner_steps):
            # Compute the logits using the current parameters
            logits = self._forward(images, parameters)
        
            # Compute the score using the logits and the true labels
            score = util.score(logits, labels)

            # Compute the loss
            loss = torch.nn.functional.cross_entropy(logits, labels)
        
            # Compute the gradients of the loss w.r.t. parameters. See autograd.grad
            input_parameters = [v[1] for k, v in enumerate(parameters.items())]
            gradient = autograd.grad(outputs=loss, inputs=input_parameters, create_graph=train)

            # Update the parameters using gradient descent
            parameters = {
                k[0]: parameters[k[0]] - self._inner_lrs[k[0]] * gradient[i]
                for i, k in enumerate(parameters.items())
            }
        
            accuracies.append(score)

        # Compute the logits with the final adapted parameters
        logits = self._forward(images, parameters)
    ...
    def _outer_step(self, task_batch, train):
        ...
        for task in task_batch:
            ...
            # For a given task, use the _inner_loop method
            params, accs, _ = self._inner_loop(images_support, labels_support, train)

            # Compute the query logits using the adapted parameters
            logits = self._forward(images_query, params)

            # Compute the query loss
            loss = torch.nn.functional.cross_entropy(logits, labels_query)

            # Compute the query accuracy
            score = util.score(logits, labels_query)

            # Add the metrics to their corresponding contianers
